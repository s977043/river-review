...
- **Carrying incompleteness into the gate is opt-in (#2320 / #2337)**: whether the fact "the review ran but never saw its subject" reaches the gate is the host's choice. Both are OFF by default, and the default gate output is unchanged bit for bit
  ...
  - `RIVER_GATE_COVERAGE=1`: a run whose `reviewCoverage.status` is `partial` or `not_executed` lands on `NO_GO` (`COVERAGE_INCOMPLETE`). This is an **independent gate input**, not a `suggestedLoopSignal` downgrade; a coverage gap must stop uniformly rather than depend on the current `decision`. **Today this takes effect on the `river run --gate` path only; on the `review exec` path it remains a no-op until the engine returns `reviewCoverage`**
    - Since #2410, `reviewCoverage` is produced not only by `--reviewers` orchestration but also by the single-reviewer path **when an LLM call was actually attempted**. A valid response (including `NO_ISSUES`) is `complete`; a transport / response-envelope / model-output parse failure becomes a failed required unit and therefore `not_executed`
    - Since #2423, a `--reviewers` role × chunk whose LLM call failed is recorded as `failed` (`reviewer_error`) instead of `completed`, and the aggregate becomes `partial` / `not_executed`
    - Intentional skips such as dry-run, offline mode, or a missing API key still emit no coverage observation. This keeps "attempted and failed" distinct from "not attempted by policy/configuration"
