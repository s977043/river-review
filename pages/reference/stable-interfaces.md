...
| Security Audit Coverage (`schemas/security-audit-coverage.schema.json`)                    | Experimental | #2267 Phase 3 の observe-only semantic coverage contract。`ReviewCoverage`・finding lifecycle・Gate とは分離し、自動 runtime emission と Gate integration はまだ行わない                                                                             |
