...
| Suppression Context (`schemas/suppression-context.schema.json`)                            | Experimental | Riverbed Memory の `type: suppression` エントリの `context` 形式（#687 / Epic #650）。利用者が `river suppression add` で `.river/memory` に書く。親エントリの形式は `schemas/riverbed-entry.schema.json` が規定する。#2202 Phase 0 で追加した `skillId` / `skillVersion` / `rulesDigest` は optional の記録専用項目で、失効判定では読まない。`river suppression add` はプロジェクトルールがあれば `rulesDigest` を書く。`--skill <id>` を指定すると `skillId` も書き、その ID の skill が見つかれば `skillVersion` も書く                                                                                                                                                                                                                                                                                                                                                                                                                     |
