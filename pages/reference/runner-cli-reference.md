...
明示指定に未知のロール名が1つでも含まれる場合、レビューを開始せずエラーにします。
...
#### `--entry` の受理範囲（Internal） {#entry-acceptance-scope}
