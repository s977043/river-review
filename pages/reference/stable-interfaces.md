...
| Review Coverage (`schemas/review-coverage.schema.json` / JSON・saved run `reviewCoverage`) | Experimental | #2212 Phase 1 の観測用 contract。既定では Gate / `decision` に影響しない。#2337 の opt-in `RIVER_GATE_COVERAGE=1` を有効にすると、`river run --gate` で `status` が `partial` / `not_executed` の run を `NO_GO`（`COVERAGE_INCOMPLETE`）へ倒す。opt-in が無ければ gate 出力は従来どおりで、`gate.inputs` に coverage key も増えない。Review Coverage は `--reviewers` の orchestration に加え、#2410 以降は単一 reviewer 経路でも **LLM call を実際に試行した場合だけ**生成する。成功（有効な `NO_ISSUES` を含む）は `complete`、transport / response-envelope / model-output parse failure は required unit `failed` → `not_executed`、dry-run / offline / API key 未設定などの意図的 skip は「観測なし」として field 自体を生成しない。したがって coverage gate の発火条件は `RIVER_GATE_COVERAGE=1` + `--gate` + 不完全な coverage 観測であり、`--reviewers` は必須ではない。`review exec` は engine が `reviewCoverage` を返すまで引き続き no-op。JSON output と `.river/runs/*.json` の field、`fileScope` は additive・optional |
