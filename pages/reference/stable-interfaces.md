...
| Review Coverage (`schemas/review-coverage.schema.json` / JSON・saved run `reviewCoverage`) | Experimental | #2212 の Experimental な実行coverage contract。既定では Gate / `decision` に影響しない。ただし `river runs diff` の loop signal は既定でも coverage を見るので、LLM 呼び出しに失敗した単一 reviewer の run は `CONVERGED` ではなく `NO_SIGNAL` になる。#2410 以降、`--reviewers` orchestration に加え、単一 reviewer でも LLM call を実際に試行した場合は `reviewCoverage` を生成する。有効な応答（`NO_ISSUES` 含む）は `complete`、transport / response / parse failure は required unit `failed` → `not_executed`、dry-run / offline / API key 未設定は観測なしとして field を省略する。`RIVER_GATE_COVERAGE=1` + `--gate` の opt-in 時だけ `partial` / `not_executed` を `NO_GO`（`COVERAGE_INCOMPLETE`）へ倒す。`review exec` は engine が `reviewCoverage` を返すまで no-op。JSON / saved-run field と `fileScope` は additive・optional |
