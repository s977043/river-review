...
| `blocked-label-guard.yml`                | Blocked Label Guard                | `pull_request`（opened / reopened / synchronize / labeled / unlabeled）/ `merge_group`                                               | `blocked` などマージ阻止ラベルが付いた PR でジョブを落とし、マージを機械的に止める                                                                          | 1 件       |
...
| `test.yml`                               | CI                                 | `push`（main）/ `pull_request` / `merge_group` / `workflow_dispatch`                                                                 | lint、単体テスト、skill schema、meta、dist 鮮度、CLI 統合、engine-only install を実行する                                                                   | 6 件すべて |
...
必須チェック 7 件のうち 6 件が `test.yml` に集中しているのは、この基準を満たすジョブが `test.yml` に固まっているからです。例外の `Blocked label guard` は `paths:` フィルタを持たず、ラベルの有無だけで決定論的に判定するため基準を満たします。

### 必須チェックは `merge_group` でも走らせる
