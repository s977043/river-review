...
- `review`
  ...
  - `viewpoints` ([#2252](https://github.com/s977043/river-review/issues/2252)): runtime mode for Skill-owned Review Viewpoint Catalogs. This setting is independent from the Prompt Compiler.
...
- `memory` ([#687](https://github.com/s977043/river-review/issues/687))
  ...
  - `suppressionRequireRulesMatch`: `false` (default). When `true`, a suppression entry whose `context.rulesDigest` (recorded at issuance) does not match the digest of the current `.river/rules.md` (plus `.river/rules.d/*.md`) stops suppressing (#2202 Phase 2). Entries without `rulesDigest`, repositories without rules files, and entries with an unknown `rulesDigestAlgo` are not judged and keep suppressing. A stopped entry is reported by a warning and by `reason: "rules-digest-mismatch"` in `reviewDebug.suppressionsApplied`.
