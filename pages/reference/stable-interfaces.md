...
| Review Coverage (`schemas/review-coverage.schema.json` / JSON・saved run `reviewCoverage`) | Experimental | #2212 Phase 1 の観測用 contract。既定では Gate / `decision` に影響しない。#2337 で opt-in `RIVER_GATE_COVERAGE=1` を追加した。有効時のみ `status` が `partial` / `not_executed` の run を gate の独立入力へ渡し、`NO_GO`（`COVERAGE_INCOMPLETE`）へ倒す。opt-in が無ければ gate 出力は従来どおりで、`gate.inputs` にキーも増えない。**現時点で実効があるのは `river run --gate` 経路のみであり、`review exec` 経路は engine が `reviewCoverage` を返すまで no-op となる**（opt-in を有効にしても止まらない）。発火条件は `RIVER_GATE_COVERAGE=1` / `--gate` / `--reviewers` の 3 つである。`--reviewers` に既定値は無く、省略した run は `generateReview` 経路へ分岐して `reviewCoverage` を生成しない。`RIVER_GATE_COVERAGE=1` + `gate: true` + 非空の `reviewers` で GitHub Action から到達できる。ロール検証は CLI が行う。JSON output と `.river/runs/*.json` への field は additive・optional。Slice C で追加した `fileScope` も optional |
