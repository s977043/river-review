...
以下のキーワードは目安として使い、依頼の明示的な範囲・変更内容・対象ファイルと合わせて専門スキルを選択します。語が偶然含まれるだけではルーティングしません。
...
複数カテゴリが実質的に該当する場合、ユーザーが指定した観点を優先する。明示指定がない場合は、以下の順で優先度を決定する:
...
同点の場合: 両カテゴリのスキルを併用する。無関係な観点まで広げず、対象範囲が重なる finding は最終出力で統合する。
...
複数カテゴリを併用する場合や、明示的な指定から別カテゴリへ振り分ける場合は、理由をユーザーに短く伝える。通常のデフォルト経路ではレビュー結果に含めればよく、毎回の進捗通知は不要。

```text
→ [river-review-security] にルーティングしました（理由: 「脆弱性」キーワード検出）
```

ユーザーが観点や範囲を変更した場合は、新しい依頼を優先してルーティングし直す。
