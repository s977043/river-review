...
| Fast Verification Evidence (`schemas/fast-verification-evidence.schema.json`)              | Experimental | #2275 PR-3B（Epic #2054 Phase 3）の `after-change` checkpoint evidence contract。`observe` 固定で Gate / `decision` / merge 権限を持たず、`pass` は決定論チェックが実際に走って成功した場合だけを指す。この evidence を書き出す host adapter はまだ無く（runtime emission なし / writer 未配線）、Gate integration も行わない |
