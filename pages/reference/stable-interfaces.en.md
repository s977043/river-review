...
| Fast Verification Evidence (`schemas/fast-verification-evidence.schema.json`)                 | Experimental | `after-change` checkpoint evidence contract from #2275 PR-3B (Epic #2054 Phase 3). It is fixed to `observe` and holds no Gate, `decision` or merge authority; `pass` means a deterministic check really ran and really succeeded on the subject revision. No host adapter writes it yet (no runtime emission, writer not wired), and Gate integration is not part of this phase |
