...
- **Migration (#2309)**: before the fix the ordinary path counted the marker as a context line, so every addition below it was recorded in `addedLines` one line too low. A line number that is now smaller by one for the same diff is that correction, not a regression.
