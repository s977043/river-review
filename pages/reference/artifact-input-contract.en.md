...
#### Diff supply routes (full enumeration against the implementation)
...
| 5   | artifact file read at `src/lib/review-plan.mjs:484` / `:841`          | the `diff` artifact via tier 1 / 2 / 3         | Host (may be real git output) | unspecified           | supported     |
| 6   | `src/lib/review-plan.mjs:481` / `:838` `diffOverride.diffText`        | `--base <ref>` on `review plan` / `exec`       | River Review (git)            | 3                     | cannot enter  |
| 7   | `src/lib/local-runner.mjs:533` `runLocalReview({ context })`          | programmatic embedding                         | Host                          | unspecified           | supported     |
| 8   | the `diffText` option at `runners/node-api/src/index.ts:342` / `:389` | Node API (`buildExecutionPlan()` / `review()`) | Host                          | unspecified           | supported     |
...
- **Combined diff**: `diff --cc` / `@@@` output is **supported** (#2294). `git show --cc` and `git log -p --cc` are real git output and arrive legitimately through routes 5 / 7 / 8. The parse layer accepts a hunk header whose marker widens by one `@` per parent (`@@@` for two parents, `@@@@` for a three-parent octopus merge) and counts each body line by its per-parent prefix COLUMNS. A line is absent from the merge result, and consumes no line number, when any column is `-`; it is recorded in `addedLines` when any column is `+`. A conflict-resolution line (`++` for two parents) belongs to neither parent and therefore reaches review, which is the point.
- **Binary files inside a combined section**: a combined section for a binary file emits only `Binary files differ` and carries no `---` / `+++` triple, so that file is not registered in `files[]` and is not reviewed. This matches the behaviour for ordinary (non-combined) binary diffs and is identical before and after #2294 (measured in a throwaway repository).
- **`\ No newline at end of file`**: the marker is metadata, not a line of the merge result, and it consumes no line number on either the combined or the ordinary path (#2309). Real git always emits it for a file with no trailing newline, so host-supplied diffs on routes 5 / 7 / 8 are treated the same way.
- **Migration (#2309)**: before the fix the ordinary path counted the marker as a context line, so every addition below it was recorded in `addedLines` one line further down the file than it really is. A line number that is now smaller by one for the same diff is that correction, not a regression.
- **Measured blast radius (#2309)**: replaying the 400 most recent commits of this repository through the parser before and after the fix changed the output of 96 of them (24%), and every one of those 96 contains this marker. No commit without the marker changed. Whether you are affected therefore comes down to whether your diff contains a file with no trailing newline.
- **Effect on v2 suppressions (#2309)**: the v2 suppressions in `src/lib/suppression-apply.mjs` are line-anchored, so one placed below the marker can stop matching once the line numbers are corrected. A suppression that stops matching shows up as the finding reappearing, so re-anchor it if you still want it suppressed. v1 fingerprints carry no line number and are unaffected.
- **Migration**: before #2294 a combined-diff input returned zero findings. The same input now returns findings. When comparing against a result recorded while those files read as empty, the difference is not a regression — it is previously invisible change becoming visible.
...
These three tiers apply only to the `river review plan` and `river review exec` surfaces. `river run`, `river skills`, and `river doctor` perform no artifact resolution and always obtain the diff from git themselves (routes 3 / 2 / 4 in "Diff supply routes" above).
