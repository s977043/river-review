...
停止が抑止される範囲は Layer 2 の signal に限られます。artifact が `gate` ブロックを持つ場合、下記の参照実装は gate を signal より優先するため、Layer 1 由来の `GO` によって `partial` の run でも停止しえます。gate へ coverage を持ち込むかは issue #2337 で扱います。
