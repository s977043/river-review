...
**ただし系統 4 との衝突は放置しない。** `review-differ.mjs` の `changeStatus: 'resolved'` は「前 run の fingerprint が今 run に無い」以上の意味を持たないことを明示します。~~この明示は Phase 1 の契約テキストで行います。~~ → 実施時期は #2325 での実装に変わりました（次段落参照）。本 ADR では `review-differ.mjs` の語をリネームしません（既存 consumer への波及が Phase 0 の範囲を超えるため）。~~リネームの要否は未決とします。~~ → リネームの要否は issue #2325 で決着しました（リネームせず qualification を足す）。
...
| 1 Contract        | 新 schema を `pages/reference/stable-interfaces.md`（ja / en）へ登録すること。issue #2316 が 30 中 20 の未登録を記録しており、新設分で穴を広げない | 未着手。ここでいう新 schema は本 Epic が Phase 1 で新設する `resolution` / `verification` / `authorResponse`（D3 の所有表を参照）であり、まだ存在しない | 登録を同一 PR の義務とする。登録の作法そのものは #2315 が `stable-interfaces.md` の「スキーマのバージョニングポリシー」節へ固定済み（publish した時点で登録し、writer 未配線ならその旨を備考に書く） |
...
同じ未完走 run は `STOP_OSCILLATED` 側からも停止を作っていました（issue #2336）。`oscillated` の absent は「fingerprint がその run に無い」という測定であり、reviewer がタイムアウトした run は finding を落とすためです。PR #2365 の対応は、絶対の降格ではなく検知条件として入れました。absent の run の coverage が `partial` / `not_executed` のとき、その absent を振動の根拠として数えません。判定を signal 層に置きませんでした。`deriveLoopSignalFromRunsDiff` から読めるのは最新 run の coverage だけであり、疑うべき absent は手前の run にあるためです。run ごとの coverage を持つのは `review-differ.mjs` の timeline 側にあたります。この条件は振動の判定を打ち消すのではなく保留するもので、コストが無いわけではありません。absent 側の run が不完全なあいだ根拠は数えられないため、coverage が恒常的に不完全な環境では `STOP_OSCILLATED` に到達しません。`reviewCoverage` が run 単位の観測であって finding を担当した unit 単位ではない点も同じ方向に働きます。`incompleteRequiredUnitIds` は `review-coverage.mjs` に存在しますが production の消費者が無く、finding 側に unit 帰属フィールドも無いため、現在の構成ではこれ以上細かい判定を作れません。
